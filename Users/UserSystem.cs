...
namespace ModUploadSite.Users
{
    public class UserSystem
    {
        ...
        public static GenericRequestResponse RequestPasswordReset(string user)
        {
            ...
            if (!emailClient.SendEmail(found.email, "Reset password", "You requested a password reset on " + Env.config.publicAddress + " . Click this link to reset your password (Due to security this link will expire in 15 minutes): " + Env.config.publicAddress + "confirmpasswordreset?token=" + token + "&username=" + found.username + "&email=" + found.email + "\n\nDidn't request a password reset? Safely ignore this email."))
            {
                return new GenericRequestResponse(500, "We were unable to send an email to you. Please try again later");
            }
            return new GenericRequestResponse(200, "Reset email sent. Check your inbox and spam for an email by " + Env.config.emailSender);
        }
        ...
        public static GenericRequestResponse ResetPasswordConfirmed(string user)
        {
            ...
            if (!emailClient.SendEmail(found.email, "Your password has been changed", "Your password for " + Env.config.publicAddress + " has been changed. If you did not change it request a password reset now! To reset your password go here " + config.publicAddress + "requestpasswordreset"))
            {
                return new GenericRequestResponse(500, "We were unable to send an email to you. Please try again later");
            }
